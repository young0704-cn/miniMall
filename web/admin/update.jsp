				...
				function fileChange(){//注意：此处不能使用jQuery中的change事件，因此仅触发一次，因此使用标签的：onchange属性
					$.ajaxFileUpload({
						...
						success: function(obj) //服务器成功响应处理函数
						{
							...
							var imgObj = $("<img>");
							//给img标签对象追加属性
							imgObj.attr("src","${pageContext.request.contextPath}/image_big/"+obj);
...
<script type="text/javascript">
	function myclose(ispage) {
		window.location="${pageContext.request.contextPath}/prod/split.action?pageNum="+ispage;
		//window.close();
	}
</script>
			<div id="table">
				<form action="${pageContext.request.contextPath}/prod/update.action" enctype="multipart/form-data" method="get" id="myform">
					<input type="hidden" value="${prod.pId}" name="pId">
					<input type="hidden" value="${prod.pImage}" name="pImage">
					<input type="hidden" value="${info.pageNum}" name="pageNum">
					<table>
						<tr>
						...
						<tr>
							...
							<td>
								<input type="reset" value="取消" class="btn btn-default" onclick="myclose(${info.pageNum})">
							</td>
						</tr>
					</table>
				</form>
