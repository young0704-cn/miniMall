...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            ...
            商品类型：<select name="typeid" id="typeid" style="width: 80px">
            <option value="-1">请选择</option>
            <c:forEach items="${ptlist}" var="pt">
                <option value="${pt.typeId}">${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            价格：<input name="lprice" id="lprice" style="width: 120px"> — <input name="hprice" id="hprice" style="width: 120px">
...
<script type="text/javascript">
    ...
    function deleteBatch() {
        ...
        if(zhi.length==0){
            ...
            <%--    window.location="${pageContext.request.contextPath}/prod/deletebatch.action?str="+str;--%>
                const id = new Array(zhi.length);

                $.each(zhi,function (index,item) {
                    id[index] =$(item).val();
                })
                alert(id);
                    window.location="${pageContext.request.contextPath}/prod/deletebatch.action?id="+id+"&pageNum="+${info.pageNum};
            }
        }
    }
    //单个删除
    ...
    function one(pid, ispage,typeId,pName,lprice,hprice) {
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        url:"${pageContext.request.contextPath}/prod/ajaxsplit.action",
            ...
            success:function () {
                //重新加载分页显示的组件table
                $("#table").load("http://localhost:8080/miniMall/admin/product.jsp #table");
            }
        })
    };
